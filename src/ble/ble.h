...
#define bleManagerTAG "bleManager"
...
/*  Page 191 of Apple's "Accessory Design Guidelines for Apple Devices (Release R20)" recommends to use only one of
  ...
  */
  ...
  // Start advertising
  ESP_LOGI(bleManagerTAG, "Sending Advertisement...");
  pAdvertising->start();

  // Turn lights off while "sleeping"
  vTaskDelay(pdMS_TO_TICKS(delayMilliseconds)); // delay for delayMilliseconds ms
  pAdvertising->stop();
